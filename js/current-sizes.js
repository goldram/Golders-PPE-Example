...
//function CurrentSizes() {
    ...
    //-------------------------------------------------------------------------------------
    // Return product item type based on item number. 
    //-------------------------------------------------------------------------------------
    function GetItemTypeDescription(i) {
    ...
    function PopulateSizesBoxItem(oBox, oData, bAllowEdit, sUpdateStatus) 
    {
        ...
        oItemBox.style.margin = "0px 0px 0px 0px";
        ...
        //Insert Item Type field (read only).
    ...
    function PopulateItemBrandDropdown(oDropdown, sType, sBrand) {
        ...
        var oOptionsAvail = {
            ...
            "Old Boots" : ["Bata", "Dawgs", "Mack", "Mongrel", "Oliver", "Steel Blue", "Other"],
            "Hats"      : ["Akubra", "Other"],
            "Old Hats"  : ["Akubra", "Other"],
            "Jackets"   : ["Stanley", "Other"],
            "Jumpers"   : ["Elegant Knitwear", "Other"],
            "Pants"     : ["Bisley", "Can't Tear 'Em", "King Gee", "Levi", "Wrangler", "Yakka", "Other"],
            "Old Pants" : ["Bisley", "Can't Tear 'Em", "King Gee", "Levi", "Wrangler", "Yakka", "Other"],
            "Shirts"    : ["Elegant Knitting", "King Gee", "Yakka", "Other"],
            "Old Shirts": ["Elegant Knitting", "King Gee", "Yakka", "Other"],
            "Socks"     : ["Bamboo", "Other"]
        }    
...
//		    
        ...
        for (var i=1; i<=3; i++) 
        {
            //Get the item type description for the item number.
            var sType = GetItemTypeDescription(i); 
            
            //If data was retrieved from the database, see if there is data for this type.
            var oSavedRecord = null;
            if (moCurrentSizes != null && moCurrentSizes.length > 0)
            {
                for (p=0; p<moCurrentSizes.length; p++)
                {
                    var sSavedType = $data("ItemType", moCurrentSizes[p]);
                    if (sSavedType == sType) 
                    {
                        oSavedRecord = moCurrentSizes[p];
                        break;    
                    }
                }
            }
            ...
            { 
                ...
                var oData = {
                    "EmployeeID"    : $data("EmployeeID", oSavedRecord),
                    "ItemNum"       : i+1,
                    "CurrentSizeID" : $data("CurrentSizeID", oSavedRecord), 
                    "ItemType"      : $data("ItemType", oSavedRecord), 
                    "ItemBrand"     : $data("ItemBrand", oSavedRecord), 
                    "ItemSize"      : $data("ItemSize", oSavedRecord) 
                }
            ...
            { 
                //Display new data with defaults.
                var oData = 
                {
                    "EmployeeID"     : msEmployeeID,
                    "ItemNum"        : i,
                    "CurrentSizesID" : 0, 
                    "ItemType"       : GetItemTypeDescription(i), 
                    "ItemBrand"      : "", 
                    "ItemSize"       : ""
                }
    ...
    function SetStatusMsg(sMsgID, sMsg, sDetails, bError, bTimeout, sColor, iTimeoutLength) 
    {
	    ...
	    {
		    oMsg.style.color = (sColor) ? sColor : "rgb(255,247,28)";  //darker green by default
		    oMsg.innerHTML = sMsg;
